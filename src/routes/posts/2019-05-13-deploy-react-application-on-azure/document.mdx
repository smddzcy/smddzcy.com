...
First, you have to create a file called `web.config` to tell Azure how to run application. Here's my `web.config` that includes the configuration for Node + React + socket.io (I use it quite often).
...
For Azure Pipelines (my preferred way), you have to create a `azure-pipelines.yml` at the project root. It's a file that basically tells Azure how to build your application:

...
Microsoft told us the SQL API is better, and we could solve all of our issues by switching to that. So we decided it's better to make the switch immediately without growing our codebase any larger. We made the switch with a couple of tricks (which I will explain in my <a href="../2019-05-15/mongooselike-orm-for-azure-cosmos-db-sql-api" title="Mongoose-like ORM for Azure Cosmos DB SQL API">next post</a>), and actually all the issues above were solved, but then we had many other issues:
